...
class JobService {
	...
	async scanOutputDirectory() {
		try {
			...
			for (const file of files) {
				...
				const exists = Array.from(this.jobs.values()).some(
					(job) => job.url === relativeUrl || job.thumbnail === relativeUrl,
				);
	...
	createJob(
		...
		file: any,
		mode: JobMode = 'video',
		encodingMode: EncodingMode = 'hardware',
	): ExtendedJob {
		const job: ExtendedJob = {
			id,
			clientId,
	...
	getUserJobs(clientId: string): ExtendedJob[] {
		return this.getAllJobs().filter(
			(job) => job.clientId === clientId || job.clientId === 'recovered',
		);
	}
